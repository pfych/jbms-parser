...
public class DTXDecoder extends ChartDecoder {
	...
	private static final DTXCommandWord[] DTX_COMMAND_WORDS = DTXCommandWord.values();
				...
				if(line.charAt(0) == '#') {
					...
					if (matchesReserveWord(line, "RANDOM")) {
						...
						if ('0' <= c && c <= '9' && line.length() > 6) {
							...
							for (DTXCommandWord cw : DTX_COMMAND_WORDS) {
								if (line.length() > cw.name().length() + 2 && matchesReserveWord(line, cw.name())) {
									DecodeLog log = cw.function.apply(model, line.substring(cw.name().length() + 3).trim());
									if (log != null) {
										this.log.add(log);
...
enum DTXCommandWord {
	...
	VOLUME ((model, arg) -> {
		try {
			// DTX Has ":" after volume declarations
			String parsedArg = arg.replace(':', ' ').trim();
			model.setVolwav(Integer.parseInt(parsedArg));
		} catch (NumberFormatException e) {
			return new DecodeLog(WARNING, "VOLUME IS FUCKED");
		}
		return null;
	}),
	...
	private DTXCommandWord(BiFunction<BMSModel, String, DecodeLog> function) {
